...
export function registerScheduleHandlers(): void {
  ...
  ipcMain.handle('schedule:createTodo', (_e, data: {
    ...
    run(
      `INSERT INTO schedule_todos (id,title,description,date,time,quadrant,task_type,tag_id,created_at,updated_at)
       VALUES (?,?,?,?,?,?,?,?,?,?)`,
      [id, data.title, data.description || '', data.date, data.time || null,
        data.quadrant ?? 1, data.taskType || 'plan', data.tagId || null, now, now]
    )
