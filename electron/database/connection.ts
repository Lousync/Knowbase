...
export function runMigrations(): void {
  ...
  if (!applied.has('038_checkin_module')) {
  ...
  if (!applied.has('040_knowledge_network')) {
    ...
    `)
    db.run("INSERT INTO _migrations (name) VALUES ('040_knowledge_network')")
  }

  if (!applied.has('040_supervise_module')) {
  ...
  if (!applied.has('046_agent_sessions')) {
