...
| **沉浸 M0** | ① 活动栏出现「Agent」入口并打开全屏工作台 ② 左栏新建任务（教学/研读/复盘模板）→ 自动发开场指令 ③ 发送/回复真实对话，活动条实时显示工具调用（中文名+第 N 次）④ 助手消息可展开「调用轨迹」⑤ 右栏「本次改动」列出文件并点击直达编辑器 ⑥ 顶部时间线/文档视图切换（文档视图=最近回复整篇）⑦ 素材/产物区为空态说明（M1 开放）⑧ 顶栏 Token chip（默认收起）：展开看模型名/本会话 tokens·轮次·工具·耗时/本月 tokens+预算条 ⑨ 进入 Agent Tab（主/副栏）时右下角 AI 轻问答浮钮自动隐藏 | ✅ 代码完成待真机 |
| **quiz 出题官** | ① 应用重启后 AI 技能页可见「出题官」（standalone skill，正式+两 dev 数据目录已装）② 对话说"考考我/出几道题"→ AI 按 quiz 围栏 JSON 出选择题，**对话内渲染为可点选答题卡** ③ 答错时 AI 讲解错因 ④ 简答/大题用 spoiler 折叠展示 ⑤ 说"存成卷子"→ agent 给出带 frontmatter(type:quiz) 卷文本（写入待确认/随 M1 产物） | ✅ 资产完成待真机 |
| **会话要求** | ① 沉浸 Tab 顶栏「会话要求」可给当前会话设置一条全局要求（如"只用中文/只聊 XX 主题"）② 对话顶部常驻提示条（可收起）③ 仅本会话生效：后续问答与「重新生成」都遵守，切换会话互不影响 ④ 轻问答侧栏机制通用（同 IPC，界面入口后续补） | ✅ 代码完成待真机 |
| **PPT 素材阅读** | ① AI 可读 PPT：`docs.read-text` 提取 .pptx 全文（既有）；.ppt 旧格式明确不支持 ② 沉浸右栏「PPT 素材」从当前仓库选 .pptx（递归 ≤3 层）加入素材 ③ 文档视图逐页阅读：页码/原文编号/文本卡（空页提示）可翻页 ④ 「讲解此页」把当前页文字发 AI 讲解 ⑤ 素材可移除 | ✅ 代码完成待真机 |
