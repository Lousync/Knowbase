...
> 状态：**P1–P8 全部实现完毕**（2026-09-06 验证）。提交：`57935d5`（P1–P5 后端+P7 主链路）→ `da773a2`（UI 接线合流）→ `6706982`（P6 整仓归档+P7 设置页入口）→ `064d4d3`（P8 标题栏切换器）。
> 验证：tsc node+web 对基线零新增；冒烟 P4 15/15、P6 18/18、P8 15/15（`tmp/smoke/`）。
> 已知遗留（明确接受，非缺陷）：legacy 根 `_attachments/` 读兼容未进 ws:readImage 白名单；回收站博客恢复仍写 sqlite；导出导入旧备份体系（exportBackupToZip 系）未并轨；quiz 插件模式表留 sqlite。
