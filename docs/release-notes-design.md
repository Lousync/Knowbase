...
- [ ] **v3.1.0 亮点文案待人工审核**（`highlights.ts`，数字均取自本仓实测；已按原型补齐分节 / 摘要行 / 折叠改动 / 演示区结构）
- [x] ~~`CHANGELOG.md` 还没有 `## v3.1.0` 小节~~ → 已补（2026-09-12，与合并内容对齐）并重跑生成器（data 38 版本 / 359 条目，契约 PASS）
- [x] ~~页面形态升级~~ → 已按 `tmp/release-notes-proto/release-notes.html` 原型改版：大版本头部（已更新 pill / 上一版 / 在线日志链接）+ 摘要行 + 分节亮点卡（可折叠「全部改动」+ 四种可重播动效演示区）+ 完整清单折叠 + 更早版本导航
- [ ] P1：动效演示区（6 个可交互 demo 组件 + `> demo:` 内联指令 + `docs.html` 忽略规则）——指**动效文档站**的演示区，与更新说明页内演示区是两回事
- [ ] 官网更新日志页与这里的口径统一（38 版本全量内联 vs 近年 + Releases 链接，待定）
