...
`npm run dev` 启动后,活动栏底部有「开发者工具」入口(打包版不存在)。左侧是工具导航,工具会持续扩充,当前包含:

**帮助文档编辑器**

- 新建 / 编辑 / 删除文档,表单填写 title / category / icon(带 lucide 图标选择与校验),右侧实时预览
- 保存直接落盘 `src/modules/help/docs/`,Vite 立即感知,帮助模块无需重启即可看到
- 文件名即文档 id(Toast `detail` 用它跳转),新建时自动跟随标题;重命名会删除旧文件并弹窗确认
- 列表中的琥珀色圆点表示该文件在 git 里有未提交改动;改动需 git 提交后才会随构建进入正式版
