...
export const SETTINGS = {
  ...
  agentCompressionEnabled: { default: true, type: 'toggle', label: '历史自动压缩', group: 'Agent 循环', desc: '上下文逼近预算（80%）时自动把较早的对话折叠为持久化纪要（代替直接丢弃），也可随时输入 /compress 手动触发；关闭后退回裁剪', keywords: ['压缩', '纪要', 'compact', 'compress', '上下文', '历史'], section: 'aiTools', ui: true, scope: 'global', level: 'normal', affects: 'live', anchor: 'aiTools.agentLoop', aiTab: 'builtin' },
