    ...
    <div className="flex flex-col h-screen bg-[var(--bg-primary)] overflow-hidden" onKeyDown={handleKeyDown} tabIndex={-1}>
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-[var(--border-color)] bg-[var(--bg-secondary)] shrink-0" style={{ WebkitAppRegion: 'drag' } as React.CSSProperties}>
        <div className="flex items-center gap-2">
          ...
          <span className="text-[12px] font-semibold text-[var(--text-primary)]">Knowbase 填充</span>
        </div>
        <button
          onClick={handleHide}
      ...
      <div className="px-3 py-2 shrink-0">
        <div className="relative">
          ...
          />
          {searchQuery && (
            <span className="absolute right-2 top-1/2 -translate-y-1/2 text-[10px] text-[var(--text-muted)]">{filtered.length}</span>
          )}
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto" style={{ WebkitAppRegion: 'no-drag' } as React.CSSProperties}>
        {loading ? (
          <div className="flex items-center justify-center py-8 text-[11px] text-[var(--text-muted)]">加载中...</div>
        ) : filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-8 text-[11px] text-[var(--text-muted)] gap-1">
            <Shield size={20} className="opacity-20 mb-1" />
            <p>{entries.length === 0 ? '密码本为空' : '无匹配结果'}</p>
          </div>
        ) : (
          <div className="py-0.5">
            {filtered.map(e => (
              <div
                ...
                onDoubleClick={() => { handleSelect(e); handleFill('all') }}
                className={`px-3 py-1.5 cursor-pointer transition-colors border-l-[3px] ${
                  selectedId === e.id
      ...
      {selected && (
        <div className="flex items-center gap-2 px-3 py-2 border-t border-[var(--border-color)] bg-[var(--bg-secondary)] shrink-0" style={{ WebkitAppRegion: 'no-drag' } as React.CSSProperties}>
          <div className="flex-1 text-[10px] text-[var(--text-muted)] truncate">
            {selected.title || '未命名'}
            {selected.account && <span className="ml-1 text-[var(--text-disabled)]">· {selected.account}</span>}
          </div>
          <button
            onClick={() => handleFill('passwordOnly')}
            className="flex items-center gap-1 px-2.5 py-1 text-[11px] text-[var(--text-secondary)] hover:text-[var(--text-primary)] border border-[var(--border-color)] rounded hover:bg-[var(--bg-hover)] transition-colors"
          >
            <Key size={11} /> 密码
          </button>
          <button
            onClick={() => handleFill('all')}
            className="flex items-center gap-1 px-2.5 py-1 text-[11px] bg-[var(--accent)] text-white rounded hover:bg-[var(--accent-hover)] transition-colors"
          >
            <ArrowRight size={11} /> 填充
          </button>
        </div>
      )}
    </div>
