...
import { ArrowLeft, Search, Eye, EyeOff, Copy, Plus, Trash2, ExternalLink, X, Check, Shield, Globe, User, Key, FileText, AtSign } from 'lucide-react'
...
export function PasswordVault({ onBack }: Props) {
  ...
  // Filtered entries (search matches title, account, username, url)
  ...
  const doSave = useCallback(async () => {
    ...
    try {
      ...
      setEntries(prev => prev.map(e => e.id === id
        ? { ...e, title: titleRef.current, account: accountRef.current, username: usernameRef.current, password: passwordRef.current, url: urlRef.current, notes: notesRef.current, updatedAt: new Date().toISOString() }
        : e
      ))
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setNewTitle(''); setNewAccount(''); setNewUsername(''); setNewPassword(''); setNewUrl(''); setNewNotes('')
      handleSelect(entry)
  ...
  // Copy helpers
  const handleCopyText = (text: string, type: 'account' | 'user' | 'pass') => {
                ...
                {filtered.map(e => (
                  ...
                  >
                    ...
                    <div className="text-[10px] text-[var(--text-muted)] truncate mt-0.5">
                      {e.account ? `${e.account}` : e.username ? `👤 ${e.username}` : ''}
                    </div>
